...
def dCCA(Y_1, Y_2, r_1=None, r_2=None, r_12=None, method=None):
    ...
    Lambda_1_inv_half[
        index_nonzero_diag_Lambda_1, index_nonzero_diag_Lambda_1
    ] = (Lambda_1.diagonal()[index_nonzero_diag_Lambda_1] ** -0.5)

    Lambda_2_inv_half = np.zeros(Lambda_2.shape)
    index_nonzero_diag_Lambda_2 = np.nonzero(Lambda_2.diagonal() > 0)
    Lambda_2_inv_half[
        index_nonzero_diag_Lambda_2, index_nonzero_diag_Lambda_2
    ] = (Lambda_2.diagonal()[index_nonzero_diag_Lambda_2] ** -0.5)

    Theta = (
        (Lambda_1_inv_half @ V_1.T @ X_1_hat)
        @ (X_2_hat.T @ V_2 @ Lambda_2_inv_half)
        / n
    )

    U_theta, D_theta, V_theta_t = sp.linalg.svd(
        Theta, full_matrices=True
    )  # D_theta is a vector
    ...
    A_mat_C = np.diag(
        0.5 * (1 - ((1 - D_theta[:r_theta]) / (1 + D_theta[:r_theta])) ** 0.5)
    )

    B_1 = X_1_hat @ (X_1_hat.T @ Gamma_1[:, :r_12]) / n
    B_2 = X_2_hat @ (X_2_hat.T @ Gamma_2[:, :r_12]) / n
    C_base = A_mat_C[:r_12, :r_12] @ (
        Gamma_1[:, :r_12].T @ X_1_hat + Gamma_2[:, :r_12].T @ X_2_hat
    )
    ...
    C_base_rtheta = A_mat_C[:r_theta, :r_theta] @ (
        Gamma_1[:, :r_theta].T @ X_1_hat + Gamma_2[:, :r_theta].T @ X_2_hat
    )
    ...
    return (
        ...
        r_12,
        D_theta[:r_12],
        np.arccos(D_theta[:r_12]) / math.pi * 180,
    )
...
def select_r12_by_MDLIC(ccor, r_1, r_2, n):
    ...
    for i in range(1, r_min + 1):
        I_MDL[i] = (
            I_MDL[i - 1]
            + n * np.log(1 - ccor[i - 1] ** 2)
            + np.log(n) * i * (r_1 + r_2 - i)
            - np.log(n) * (i - 1) * (r_1 + r_2 - (i - 1))
        )

    r_12 = (
        np.argmin(I_MDL[1 : (r_min + 1)]) + 1
    )  # assume the common rank >= 1.
